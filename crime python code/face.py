...
while True:
    ...
    for (top, right, bottom, left), name in zip(face_locations, face_names):
        ...
        connectiondb = mysql.connector.connect(host="localhost",user="root",passwd="",database="crimetrack")	
        cursordb = connectiondb.cursor()
        sql = "select * from wrngtemp where name =%s"
        cursordb.execute(sql,[(name)])
        results = cursordb.fetchall()
        if results:
            for i in results:
                cv2.putText(frame, name+" criminal suspected ", (left + 6, bottom - 6), font, 1.0, (255, 255, 255), 1)
        else:
            if name=="Unknown":
                    cv2.putText(frame, name, (left + 6, bottom - 6), font, 1.0, (255, 255, 255), 1)                    
            else:
                cv2.putText(frame, name, (left + 6, bottom - 6), font, 1.0, (255, 255, 255), 1)
                sql = "insert into wrngtemp (name,sidate,sitm) VALUES (%s, %s, %s)"
                val = (name,dt,pntt)
